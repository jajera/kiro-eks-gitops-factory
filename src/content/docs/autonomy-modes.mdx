...
<p>
  <Tooltip term="kiro" label="Kiro" /> runs a session in one of two modes. <Tooltip term="autopilot" label="Autopilot" /> is the default: it applies its edits and yields when the turn is done. <Tooltip term="supervised" label="Supervised" /> yields for approval after each turn that edits files, and presents the changes as individual hunks you accept or reject. [Onboard an app (Lab C)](../lab-c/) ran in Autopilot, which is why the whole scaffold appeared at once rather than a file at a time.
</p>
